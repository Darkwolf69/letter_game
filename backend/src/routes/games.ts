...
function getAuthenticatedUser(req: express.Request, res: Response): Express.User | null {
...
router.post("/", async (req, res) => {
  ...
  if (!secondUserId) {
    res.status(400).json({ message: "A második játékos azonosítója kötelező." });
    return;
  }

  if (secondUserId === user.id) {
    res.status(400).json({ message: "A két játékos nem lehet ugyanaz a felhasználó." });
    return;
  }
